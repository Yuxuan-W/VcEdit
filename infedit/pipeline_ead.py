...
def ddcm_sampler(scheduler, x_s, x_t, timestep, e_s, e_t, x_0, noise, eta, to_next=True):
    ...
    if len(scheduler.timesteps) > 1:
        prev_xt = alpha_prod_t_prev ** (0.5) * pred_x0 + noise
        prev_xs = alpha_prod_t_prev ** (0.5) * x_0 + noise
    else:
        prev_xt = pred_x0
        prev_xs = x_0

    if to_next:
        raise ValueError("to_next is banned by YUXUAN")
        scheduler._step_index += 1
    return prev_xs, prev_xt, pred_x0, alpha_prod_t_prev
...
class EditPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            token_length = text_inputs.attention_mask.sum(dim=1).item()
        ...
        return (prompt_embeds, negative_prompt_embeds), token_length
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # todo: multi-view consistent latents
                ...
                _, latents, pred_x0 = ddcm_sampler(
                  self.scheduler, source_latents,   # source_latents: zt_src
                  latents, t,   # latents: zt_tgt
                  source_noise_pred, noise_pred,    # source_noise_pred: e_src, noise_pred: e_tgt
                  clean_latents, noise=noise,   # clean_latents: z0 (use z0 and zt_src to get e_con), noise: next_iter_e
                  eta=eta, to_next=False, 
                  **extra_step_kwargs
                )

                # import matplotlib.pyplot as plt
                # plt.imshow(pred_x0[0].detach().cpu().numpy().transpose(1, 2, 0))
                # plt.show()

                # mutual branch is processed the same as target branch
                # TODO: the source_latents is computed by mutual branch
                # in mutual branch, the source latents still use et, instead of just econ
                source_latents, mutual_latents, _ = ddcm_sampler(
        ...
        image = torch.stack(
            [self.image_processor.denormalize(image[i]) if do_denormalize[i] else image[i] for i in range(image.shape[0])]
        )
